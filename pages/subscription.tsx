...
export default function Home() {
  ...
  const handleClick = (price: number, name: string) => {
    ...
    if(selectedMealkits.filter(v=>v.name === name).length < 1){
      ...
      let patch = selectedMealkits.filter(v=>v.name===name).map(v=>({...v, count:v.count + 1}))[0];
      setSelectedMealkits(selectedMealkits.map(v=>{
  ...
  const removeKit = (name: string, price: number) => {
    let count = selectedMealkits.filter((kit) => kit.name === name).map(v=>v.count)[0]; // 배열안의 첫번째 요소를 선택
    setSelectedMealkits((prevMealkits) =>
    ...
    <>
      <div className="flex-col justify-items-center">
        ...
        <VStack>
          ...
          <div className="text-lg border-t border-gray-600 mt-10 mb-4">Your Products:</div>
          <div className="flex">
            {selectedMealkits.map((kit, index) => (
              <div key={index} className="flex items-center mr-3 mb-4 bold border-b border-gray-600">
                {kit.name}({kit.count})
                <CloseButton
                  size="sm"
